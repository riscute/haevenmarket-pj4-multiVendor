...
public class MyAccountController {
	...
	public String orderSearch(@PathVariable("value") String value,Authentication authentication, ModelMap modelMap) {
		if(value.equalsIgnoreCase("all")) {
			modelMap.put("order", ordersService.findByUserId(userService.findByUsername(authentication.getName()).getUserId()));
			System.out.println(userService.findByUsername(authentication.getName()).getUserId());
			modelMap.put("pan", 1);
		}else if(value.equalsIgnoreCase("tocancelled")) {
			modelMap.put("order", ordersService.findByUserIdAndStatusId2(userService.findByUsername(authentication.getName()).getUserId(),1));
			modelMap.put("pan", 6);
		}else if(value.equalsIgnoreCase("tocompleted")) {
			modelMap.put("order", ordersService.findByUserIdAndStatusId2(userService.findByUsername(authentication.getName()).getUserId(),2));
			modelMap.put("pan", 5);
		}else if(value.equalsIgnoreCase("topay")) {
			modelMap.put("order", ordersService.findByUserIdAndStatusId2(userService.findByUsername(authentication.getName()).getUserId(),9));
			modelMap.put("pan", 2);
		}else if(value.equalsIgnoreCase("toreceive")) {
			modelMap.put("order", ordersService.findByUserIdAndStatusId2(userService.findByUsername(authentication.getName()).getUserId(),6));
			modelMap.put("pan", 4);
		}else if(value.equalsIgnoreCase("toship")) {
			modelMap.put("order", ordersService.findByUserIdAndStatusId2(userService.findByUsername(authentication.getName()).getUserId(),8));
			modelMap.put("pan", 3);
		}
		return "user/myaccount/orderhistory";
	}
